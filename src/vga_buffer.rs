...
fn test_print_something() {
    ...
    // 测试对于无法打印的字符的处理逻辑
    // writer.write_string("W哈ld!");

    write!(writer,"The number are {} and {}\n zhenghan2",42,1.0/3.0).unwrap();
...
/* pub static WRITER: Writer = Writer {
    ...
    // 编译器目前还不能在编译时直接转换裸指针到变量的引用
    // dereferencing raw mutable pointers in statics is unstable
    buffer: &mut unsafe {
...
fn test_println_many() {
    serial_print!("test println_many...");
    for _ in 0..300 {
...
fn test_println_output() {
    serial_print!("test println_output...");
